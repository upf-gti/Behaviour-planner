/*Interface - Timeline, Tracks and Clips editors*/
/**
 * Dependencies:
 *  Timeline
 *  LiteGUI
...
var TIMELINE_EDITOR = {
	...
	init: function()
	{
		...
		timeline.sidebar_width = 120;
		timeline.top_margin = 20;
		timeline.size = [600,300];
		timeline.scrollable_height = 600;
		timeline._end_time = 5;
		timeline.extended = true;
		timeline.opacity = 0.7;
	...
	onDrawTimelineContent: function(ctx, start_time, end_time, timeline )
	{
		...
		var track_height = this.track_height//TIMELINE_EDITOR.track_height;
		...
		if( this.timeline_mode == "tracks" )
		{
			...
			for(var i = 0; i < this._visible_tracks.length; ++i)
			{
				...
				ctx.save()
				ctx.fillStyle = color;
				ctx.fillRect(0,y,ctx.canvas.width,y+TIMELINE_EDITOR.track_height)
				ctx.fillStyle = "black";
				ctx.restore();
		...
		if( container )
			for(var i = 0; i < container.length; ++i)
			{
				...
				else if( this.timeline_mode == "clip" )
				{
					...
					//color = selection_color = TIMELINE_EDITOR.CC_COLORS[ i % TIMELINE_EDITOR.CC_COLORS.length];
				}
				if( name )
				{
					ctx.fillStyle = color;
				...
				if( this.timeline_mode == "clip" )
				{
					...
					ctx.fillStyle = TIMELINE_EDITOR.CC_COLORS[ i % TIMELINE_EDITOR.CC_COLORS.length ];
		...
		{
			var track_height = TIMELINE_EDITOR.track_height;
